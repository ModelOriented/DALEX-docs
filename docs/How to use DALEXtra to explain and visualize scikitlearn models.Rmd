...
```{r}
library(DALEXtra)
explainer_1 <- explain_scikitlearn(pkl_SGDC, data = titanic_test[,1:17], y = titanic_test[,18])
```
...
```{r message=FALSE}
library(ingredients)
explainer_2 <- explain_scikitlearn(pkl_gbm, condaenv = "myenv", data = titanic_test[,1:17], y = titanic_test[,18])
...
Here situation is slightly more complicated. Due to problmes in integration R with unix shell, adding Anaconda to PATH does not change anything. That is way it is required to pass path where Anaconda may be found (eg. ./anaconda3) along with yml file path. `condaenv` argument should be used for this purpose as when `yml` argument is specified and OS is Unix, `condaenv` means path to conda and any non-path argument (or NULL) will rise an error.
