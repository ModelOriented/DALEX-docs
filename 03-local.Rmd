...
The `breakDown` package [@breakDown] explains components of model prediction for a single observation. Right now it's working for `lm` and `glm` models. Break Down Plots are inspired by waterfall plots created by [`xgboostExplainer` package](https://github.com/AppliedDataSciencePartners/xgboostExplainer). 
...
wine_lm_model3 <- lm(quality ~ citric.acid +  sulphates + alcohol,
               data = wine)
wine_lm_explainer3 <- explain(wine_lm_model3, data = wine)

wine_lm_model4 <- lm(quality ~ pH + residual.sugar + sulphates + alcohol,
               data = wine)
wine_lm_explainer4 <- explain(wine_lm_model4, data = wine)
