...
Lets summarize what has happened in previous sections
...
* Section \@ref(variableResponse) shows that the relation between `construction_year` and price of square meter is non linear. 

In this section we show how to improve the basic linear model by feature engineering of `construction_year`. Findings from the random forest models will help to create a new feature for the linear model.
...
Results presented above show that the `apartments_lm_model_improved` model is much better than the two that we have considered in the chapter \@ref(modelUnderstanding).

In this use-case we showed that explainers implemented in DALEX help to better understand the model and this knowledge may be used to create a better final model.
