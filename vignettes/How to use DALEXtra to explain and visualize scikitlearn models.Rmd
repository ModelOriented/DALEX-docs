...
Here situation is slightly more complicated. Due to problmes in integration R with unix shell, adding Anaconda to PATH does not change anything. That is way it is recommended to pass path where Anaconda may be found (eg. ./anaconda3) along with yml file path. `condaenv` argument should be used for this purpose as when `yml` argument is specified and OS is Unix, `condaenv` means path to conda and any non-path argument (or NULL) will rise an error. If user left `condaenv` as NULL, DALEXtra will try to look for Anaconda on its own.
