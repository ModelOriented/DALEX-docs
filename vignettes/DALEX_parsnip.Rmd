...
In this vignette we will show explanations for models from [parsnip](https://cran.r-project.org/web/packages/parsnip/index.html) (TBD).
...
```{r, results="hide"}
bt_fited <- bt_model%>% fit(fare ~ ., data = titanic_r[train_index,])
nn_fited <- nn_model%>% fit(fare ~ ., data = titanic_r[train_index,])
svm_fited <- svm_model%>% fit(fare ~ ., data = titanic_r[train_index,])
```
...
explainer_regr_bt <- DALEX::explain(bt_fited, data=titanic_r_test, y=titanic_r_test$fare, predict_function = custom_predict, label="bt")
explainer_regr_nn <- DALEX::explain(nn_fited, data=titanic_r_test, y=titanic_r_test$fare,
predict_function = custom_predict, label="nn")
explainer_regr_svm <- DALEX::explain(svm_fited, data=titanic_r_test, y=titanic_r_test$fare,
...
rf_fited <- rf_model%>% fit(survived ~ ., data = titanic_num[train_index,])
lr_fited <- lr_model%>% fit(survived ~ ., data = titanic_num[train_index,])
svm_fited <- svm_model%>% fit(survived ~ ., data = titanic_num[train_index,])
```
...
explainer_classif_rf <- DALEX::explain(rf_fited, data=titanic_num_test, y=y_test, label= "rf", predict_function = custom_predict_classif)
explainer_classif_lr <- DALEX::explain(lr_fited, data=titanic_num_test, y=y_test, label="lr", predict_function = custom_predict_classif)
explainer_classif_svm <- DALEX::explain(svm_fited, data=titanic_num_test, y=y_test, label ="svm", predict_function = custom_predict_classif)
