...
```{r}
titanic_r <- na.omit(select(titanic, -c(country))%>%filter(fare >0))
head(titanic_r)
```
...
explainer_regr_bt <- DALEX::explain(bt_fitted, data=titanic_r_test, y=titanic_r_test$fare, predict_function = custom_predict, 
                                    label="bt", colorize = FALSE)
explainer_regr_nn <- DALEX::explain(nn_fitted, data=titanic_r_test, y=titanic_r_test$fare,
                                    predict_function = custom_predict, label="nn", colorize = FALSE)
explainer_regr_svm <- DALEX::explain(svm_fitted, data=titanic_r_test, y=titanic_r_test$fare,
                                     predict_function = custom_predict, label="svm", colorize = FALSE)
...
```{r}
vi_regr_bt <- variable_importance(explainer_regr_bt, loss_function = loss_root_mean_square)
...
```{r}
vi_regr_bt <- variable_importance(explainer_regr_bt, loss_function = loss_root_mean_square, type="difference")
...
mpp_regr_bt  <- variable_response(explainer_regr_bt, variable =  "class", type = "factor")
mpp_regr_nn  <- variable_response(explainer_regr_nn, variable =  "class", type = "factor")
mpp_regr_svm  <- variable_response(explainer_regr_svm, variable =  "class", type = "factor")

plot(mpp_regr_bt, mpp_regr_nn, mpp_regr_svm)
```

We can note that svm model, in contrast to neural net and boosting tree, does not see any difference between 1st and 2nd class which may be a bit surprising.
...
```{r}
pb_bt <-  prediction_breakdown(explainer_regr_bt, observation = filter(titanic_r_test, 
                                                                          gender == "female",
                                                                          age == 27,
                                                                          class == "1st")[1,])
pb_nn <-  prediction_breakdown(explainer_regr_nn, observation = filter(titanic_r_test, 
                                                                          gender == "female",
                                                                          age == 27,
                                                                          class == "1st")[1,])
pb_svm <-  prediction_breakdown(explainer_regr_svm, observation = filter(titanic_r_test, 
                                                                          gender == "female",
                                                                          age == 27,
                                                                          class == "1st")[1,])
plot(pb_bt, pb_nn, pb_svm)
...
explainer_classif_rf <- DALEX::explain(rf_fitted, data=titanic_num_test, y=y_test, label= "rf", predict_function = custom_predict_classif, colorize = FALSE)
explainer_classif_lr <- DALEX::explain(lr_fitted, data=titanic_num_test, y=y_test, label="lr", predict_function = custom_predict_classif,
                                       colorize = FALSE)
explainer_classif_svm <- DALEX::explain(svm_fitted, data=titanic_num_test, y=y_test, label ="svm", 
                                        predict_function = custom_predict_classif, colorize = FALSE)
...
Function `variable_importance()` computes variable importances. Output may be plotted with generic function `plot()`

```{r}
vi_classif_rf <- variable_importance(explainer_classif_rf, loss_function = loss_root_mean_square)
vi_classif_lr <- variable_importance(explainer_classif_lr, loss_function = loss_root_mean_square)
...
For more details of methods described in this section see [Variable response section in DALEX docs](https://pbiecek.github.io/DALEX_docs/3-3-variableResponse.html).
...
```{r}
pdp_classif_rf  <- variable_response(explainer_classif_rf, variable = "fare", type = "pdp")
pdp_classif_lr  <- variable_response(explainer_classif_lr, variable = "fare", type = "pdp")
pdp_classif_svm  <- variable_response(explainer_classif_svm, variable = "fare", type = "pdp")
...
Function `variable_response()` with the parameter `type = "ale"` calls `ALEPlot::ALEPlot()` function to calculate the ALE curve for the variable `fare`.

```{r}
ale_classif_rf  <- variable_response(explainer_classif_rf, variable = "fare", type = "ale")
ale_classif_lr  <- variable_response(explainer_classif_lr, variable = "fare", type = "ale")
ale_classif_svm  <- variable_response(explainer_classif_svm, variable = "fare", type = "ale")
...
```{r}
pb_rf <-  prediction_breakdown(explainer_classif_rf, observation = filter(titanic_num_test, 
                                                                          gender == "female",
                                                                          age == 2,
                                                                          class == "3rd")[1,])
pb_lr <-  prediction_breakdown(explainer_classif_lr, observation = filter(titanic_num_test, 
                                                                          gender == "female",
                                                                          age == 2,
                                                                          class == "3rd")[1,])
pb_svm <-  prediction_breakdown(explainer_classif_svm, observation = filter(titanic_num_test, 
                                                                          gender == "female",
                                                                          age == 2,
                                                                          class == "3rd")[1,])
plot(pb_rf, pb_lr, pb_svm)
